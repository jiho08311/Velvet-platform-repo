import Link from "next/link"
import type { ReactNode } from "react"
import {
  PayoutEmptyState,
  PayoutList,
} from "@/modules/payout/public/payout-dashboard-ui"
import { SUBSCRIPTION_PRICES } from "@/modules/subscription/public/subscription-price"
import { Card } from "@/shared/ui/Card"
import {
  requestPayoutAction,
  updateSubscriptionPriceAction,
} from "./dashboard-page-actions"
import type { DashboardPageData } from "./dashboard-page-data"

type PageHeaderProps = {
  title: string
  description: string
  className?: string
  descriptionClassName?: string
}

type SummaryStatProps = {
  label: string
  value: ReactNode
  help?: string
  helpClassName?: string
  children?: ReactNode
}

function PageHeader({
  title,
  description,
  className,
  descriptionClassName = "mt-1 text-sm text-zinc-500",
}: PageHeaderProps) {
  return (
    <div className={className}>
      <h1 className="text-2xl font-semibold text-white">{title}</h1>
      <p className={descriptionClassName}>{description}</p>
    </div>
  )
}

function SummaryStat({
  label,
  value,
  help,
  helpClassName = "mt-2 text-sm text-zinc-500",
  children,
}: SummaryStatProps) {
  return (
    <>
      <p className="text-sm text-zinc-500">{label}</p>
      <p className="mt-2 text-2xl font-semibold text-white">{value}</p>
      {help ? <p className={helpClassName}>{help}</p> : null}
      {children}
    </>
  )
}

export function DashboardPageView({ data }: { data: DashboardPageData }) {
  if (data.kind === "pending") {
    return (
      <main className="min-h-screen bg-zinc-950 text-zinc-100 flex items-center justify-center">
        <PageHeader
          title="승인 대기중입니다"
          description="현재 크리에이터 신청이 검토 중입니다. 승인 후 기능을 이용할 수 있습니다."
          className="max-w-md text-center"
          descriptionClassName="mt-3 text-sm text-zinc-500"
        />
      </main>
    )
  }

  if (data.kind === "empty") {
    return <PayoutEmptyState />
  }

  const {
    creator,
    payoutSummary: summary,
    payouts: payoutHistory,
    subscribersMetric,
  } = data.dashboard
  const requestEligibility = summary.requestEligibility

  return (
    <main className="min-h-screen bg-zinc-950 text-zinc-100">
      <div className="mx-auto max-w-5xl space-y-6 px-4 py-8">
        <PageHeader
          title="정산"
          description="수익 확인 및 출금을 관리하세요"
        />

        <div className="grid gap-4 md:grid-cols-2">
          <Card>
            <SummaryStat
              label="구독 가격"
              value={creator.displaySubscriptionPrice}
              help="월 구독 가격을 설정하세요"
            >
              <form
                action={updateSubscriptionPriceAction}
                className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4"
              >
                {SUBSCRIPTION_PRICES.map((price) => {
                  const isActive = creator.subscriptionPrice === price
                  const isLongPrice = price >= 10000

                  return (
                    <button
                      key={price}
                      type="submit"
                      name="price"
                      value={price}
                      className={
                        isActive
                          ? "w-full rounded-2xl border border-[#C2185B] bg-[#C2185B] px-4 py-2 text-left text-sm font-semibold text-white"
                          : "w-full rounded-2xl border border-zinc-800 bg-zinc-950 px-4 py-2 text-left text-sm font-semibold text-white transition hover:bg-zinc-900"
                      }
                    >
                      <span
                        className={
                          isLongPrice
                            ? "-translate-x-1 inline-block"
                            : "inline-block"
                        }
                      >
                        ₩{price.toLocaleString()}
                      </span>
                    </button>
                  )
                })}
              </form>
            </SummaryStat>
          </Card>

          <Card>
            <SummaryStat
              label="출금 가능 금액"
              value={summary.displayRequestableBalance}
              help="현재 출금 가능한 전액 기준으로 요청됩니다."
            >
              <form action={requestPayoutAction} className="mt-4">
                <input type="hidden" name="currency" value={summary.currency} />
                <button
                  type="submit"
                  disabled={!requestEligibility.isEligible}
                  className="w-full rounded-2xl bg-[#C2185B] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[#D81B60] active:bg-[#AD1457] disabled:cursor-not-allowed disabled:bg-zinc-800 disabled:text-zinc-500"
                >
                  전액 출금 요청
                </button>
              </form>
            </SummaryStat>
          </Card>
        </div>

        <div className="grid gap-4 md:grid-cols-1">
          <Card>
            <SummaryStat
              label="출금 요청 중 금액"
              value={summary.displayRequestedPayoutAmount}
            />
          </Card>
        </div>

        <div className="grid gap-4 md:grid-cols-1">
          <Card>
            <Link
              href="/dashboard/subscribers"
              className="block rounded-2xl p-4 transition hover:bg-zinc-900"
            >
              <SummaryStat
                label="구독자 관리"
                value={subscribersMetric.displayValue}
                help="구독자 목록 확인 및 메시지 보내기"
                helpClassName="mt-1 text-sm text-zinc-500"
              />
            </Link>
          </Card>
        </div>

        <Card>
          <div className="mb-4">
            <p className="text-lg font-semibold text-white">출금 내역</p>
            <p className="text-sm text-zinc-500">
              전체 출금 기록을 확인하세요
            </p>
          </div>

          {payoutHistory.length === 0 ? (
            <PayoutEmptyState />
          ) : (
            <PayoutList payouts={payoutHistory} />
          )}
        </Card>
      </div>
    </main>
  )
}
