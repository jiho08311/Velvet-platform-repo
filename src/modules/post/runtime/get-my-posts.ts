...
const blocksMap = new Map<string, typeof blockRows>()
  ...
  const items = await Promise.all(
    ...
    blockRows: toRenderBlockRows(blocksMap.get(post.id) ?? []),
        mediaItems: media,
      })
...
type BridgePostBlockRows = Awaited<ReturnType<typeof readPostBlockAuthority>>

function toRenderBlockRows(
  rows: BridgePostBlockRows
): Parameters<typeof buildPostRenderReadModel>[0]["blockRows"] {
  return rows
    .filter(
      (row) =>
        row.type === "text" ||
    ...
    .map((row) => ({
      ...row,
      type: row.type as "text" | "image" | "video" | "audio" | "file",
    }))
}