...
export async function getExplorePosts(
  ...
  const postsWithMedia = visiblePosts.filter((post) => firstMediaMap.has(post.id))
  ...
  const blocksMap = new Map<string, DiscoveryPostLinkItem["blocks"]>()
  ...
  const shuffledPosts = shuffleArray(postsWithMedia).slice(0, safeLimit)
  ...
  return Promise.all(
    shuffledPosts.map(async (post) => {
      ...
      const fallbackBlocks: DiscoveryPostLinkItem["blocks"] =
