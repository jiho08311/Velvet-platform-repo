...
const SQL_PATH = join(
  process.cwd(),
  "docs/db-migration/briefs/wave-010-fel-br-079-manual-db-change-pack.sql"
)
const RUNTIME_PATH = join(
  process.cwd(),
  "src/shared/canonical/entitlement/entitlement-drift-arbitration-runtime.ts"
)
const REPOSITORY_PATH = join(
  process.cwd(),
  "src/shared/canonical/entitlement/canonical-entitlement-drift-arbitration-repository.ts"
)
...
test("FEL-BR-079 introduces additive entitlement drift arbitration topology", () => {
  const sql = read(SQL_PATH)

  for (const table of [
    "canonical_entitlement_drifts",
  ...
  assert.match(sql, /entitlement_drift_key text not null unique/)
  assert.match(sql, /entitlement_drift_ordering_key text not null unique/)
  assert.match(sql, /entitlement_drift_provenance_key text not null unique/)
  assert.match(sql, /entitlement_drift_reconstruction_metadata_key text not null unique/)
})

test("FEL-BR-079 schema preserves entitlement authority and blocks forbidden mutation", () => {
  const sql = read(SQL_PATH)
  ...
  assert.match(sql, /governance_entitlement_mutation_allowed boolean not null default false/)
  assert.match(sql, /replay_entitlement_execution_allowed boolean not null default false/)
  ...
  assert.match(sql, /replay_read_only boolean not null default true/)
...
test("FEL-BR-079 runtime is feature-gated, exported, no-throw, and fail-open", () => {
  const runtime = read(RUNTIME_PATH)
  const repository = read(REPOSITORY_PATH)
  const flags = read(FLAGS_PATH)
  const index = read(INDEX_PATH)

  assert.match(flags, /WAVE_010_ENTITLEMENT_DRIFT_ARBITRATION_KILL_SWITCH/)
  assert.match(flags, /isWave010EntitlementDriftArbitrationEnabled/)
  assert.match(runtime, /synchronizeEntitlementDriftArbitrationNoThrow/)
  assert.match(runtime, /validateEntitlementDriftArbitrationReadiness/)
  assert.match(repository, /writeCanonicalEntitlementDriftArbitrationNoThrow/)
  assert.match(index, /synchronizeEntitlementDriftArbitrationNoThrow/)
...
test("FEL-BR-079 connects entitlement event runtime as advisory drift arbitration only", () => {
  const entitlementEventRuntime = read(ENTITLEMENT_EVENT_RUNTIME_PATH)

  assert.match(entitlementEventRuntime, /synchronizeEntitlementDriftArbitrationNoThrow/)
  assert.match(
  ...
  assert.match(entitlementEventRuntime, /entitlementDriftArbitrationAdvisoryOnly: true/)
  assert.match(entitlementEventRuntime, /governanceEntitlementMutationAllowed: false/)
  assert.match(entitlementEventRuntime, /replayEntitlementExecutionAllowed: false/)
  assert.match(entitlementEventRuntime, /reconciliationRepairAllowed: false/)
...
test("FEL-BR-079 repository writes only entitlement drift arbitration tables", () => {
  const repository = read(REPOSITORY_PATH)

  assert.match(repository, /\.from\("canonical_entitlement_drifts"\)/)
  assert.match(repository, /\.from\("canonical_entitlement_drift_ordering"\)/)
  assert.match(repository, /\.from\("canonical_entitlement_drift_provenance"\)/)
  assert.match(
    repository,
